# Investigation Notes — Suspicious Process Creation & Parent-Child Analysis

## Investigation Summary

This investigation examined Windows process creation and parent-child relationships on `DESKTOP-9MMM37V` using Elastic endpoint telemetry.

The activity was intentionally generated from PowerShell using `cmd.exe` to launch two standard Windows utilities:
...
The investigation focused on reconstructing the process chain, validating parent-child relationships, reviewing command-line arguments, identifying user context, and determining whether the expected process events were present in Elastic telemetry.

## Investigation Objectives

- Identify controlled process creation events.
- Determine parent and child processes.
- Validate relationships using process IDs.
- Examine command-line arguments.
- Compare controlled activity with existing endpoint activity.
- Investigate missing or incomplete process telemetry.
- Avoid conclusions unsupported by endpoint evidence.
...
The controlled activity demonstrated execution through `cmd.exe`.

## Conclusion

The investigation confirmed that Elastic can provide useful process ancestry information for Windows command-shell activity.

The strongest evidence came from combining process names, parent names, PIDs, command lines, user context, executable paths, and timestamps.

The missing standalone `hostname.exe` event also demonstrated an important SOC principle:

> When telemetry is incomplete, document exactly what was observed and avoid turning expected behavior into an unsupported claim.

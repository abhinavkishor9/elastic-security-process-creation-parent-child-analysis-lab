# Elastic Security Lab 04 — Suspicious Process Creation & Parent-Child Analysis
...
- Understand process creation from a SOC investigation perspective.
- Analyze Windows parent-child process relationships.
- Validate process ancestry using PIDs and process names.
- Examine command-line arguments.
- Identify the user and host associated with process execution.
- Validate executable paths.
- Distinguish controlled activity from unrelated endpoint activity.
- Investigate unexpected or unusual parent-child relationships.
- Identify telemetry limitations when expected process events are missing.
- Map the observed behavior to MITRE ATT&CK.
- Make an evidence-based assessment without treating unusual process chains as automatically malicious.

## Scenario

A SOC analyst is reviewing endpoint telemetry for process activity that may indicate suspicious execution.

The analyst wants to determine whether a process relationship is expected or requires further investigation. Rather than focusing on a single executable, the investigation reconstructs the process chain and examines the surrounding evidence.

Controlled commands were executed from PowerShell:
...
The purpose was to generate known-good process creation telemetry and validate whether Elastic captured the complete process chain.
...
The investigation model used in this lab was:
...
A parent-child relationship that looks unusual should become an **investigation candidate**, not an automatic malicious finding.
...
Elastic successfully captured the main controlled process chain involving PowerShell, `cmd.exe`, and `whoami.exe`.

The `hostname.exe` command was visible in the `cmd.exe` command line, although a standalone `hostname.exe` telemetry event was not observed.

The investigation also demonstrated the importance of separating controlled activity from unrelated endpoint events and validating process relationships through timestamps, PIDs, process names, command lines, user context, and executable paths.

No malicious behavior or compromise was demonstrated.
